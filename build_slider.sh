...
DEFAULT_CONFIG="gs101/private/gs-google/build.config.slider"
...
if [ "${MAKE_CORE_KERNEL}" = "0" ]; then
  ...
  if [ "${BUILD_STAGING_KERNEL}" = "1" ]; then
    ...
    GKI_KERNEL_BUILD_CONFIG=gs101/aosp-staging/build.config.gki.aarch64
  else
    GKI_KERNEL_OUT_DIR=android13-5.10
    GKI_KERNEL_BUILD_CONFIG=gs101/aosp/build.config.gki.aarch64
  fi
fi
